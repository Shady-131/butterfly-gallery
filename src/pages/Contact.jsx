...
import { G, FONT, SERIF, SOCIAL_MEDIA, buildWaUrl } from '../constants/data';

// Props: lang, tr, isRTL, showToast, social, waNumber
export default function Contact({ lang, tr, isRTL, showToast, social = SOCIAL_MEDIA, waNumber }) {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '60px 20px', fontFamily: FONT, direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      <style>{`
        ...
        @media (max-width: 640px) { .contact-grid { grid-template-columns: 1fr; gap: 24px; } }
      `}</style>
      <div className="contact-grid">
        ...
        <div>
          <div style={{ background: G.pinkL, borderRadius: 10, padding: 28, border: `1px solid ${G.bdr}`, marginBottom: 16 }}>
            {[
              { icon: <MessageCircle size={18} color={G.gold} />, text: `+${waNumber}` },
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[
              { bg: '#E1306C', icon: <InstagramIcon size={18} />, label: 'Instagram', href: social.instagram || SOCIAL_MEDIA.instagram },
              { bg: '#1877F2', icon: <FacebookIcon  size={18} />, label: 'Facebook',  href: social.facebook  || SOCIAL_MEDIA.facebook  },
              { bg: '#000',    icon: <TikTokIcon    size={18} />, label: 'TikTok',    href: social.tiktok    || SOCIAL_MEDIA.tiktok    },
            ].map(s => (
